...
"""
WeCafe Cleaning Bot (Telegram) — python-telegram-bot 21.x (Render webhook/polling)

What this version fixes (per your 3 issues + new requirements):
1) Photos from cleaning tasks are forwarded to the "Контроль" group (CONTROL_GROUP_ID) with caption.
...
TZ_NAME = (os.getenv("TIME_ZONE") or os.getenv("TZ") or "Asia/Krasnoyarsk").strip()  # Norilsk
TZ = ZoneInfo(TZ_NAME)
...
def start_health_server_polling():
    try:
...
async def mark_wait_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Accept either a regular Telegram photo or an image sent as a document ("file")
    file_id = None
    if msg.photo:
    ...
    if not file_id:
        await msg.reply_text("Нужно фото (или картинку как файл). Пришли ещё раз.")
        return MARK_WAIT_PHOTO
...
async def report_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if msg.photo:
        ...
        file_id = msg.document.file_id
    if not file_id:
        await msg.reply_text("Нужно фото чека (можно отправить как фото или как файл-картинку).")
        return REPORT_PHOTO
...
async def report_done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if CONTROL_GROUP_ID:
        ...
        for i, fid in enumerate(photos, start=1):
            try:
                await context.bot.send_photo(chat_id=CONTROL_GROUP_ID, photo=fid, caption=f"🧾 Чек {i} | {point} | {day} | {name}")
            except Exception:
                pass
...
def build_app() -> Application:
    ...
    mark_conv = ConversationHandler(
        ...
        states={
            ...
            MARK_WAIT_PHOTO: [MessageHandler(filters.PHOTO | filters.Document.IMAGE, mark_wait_photo)],
        },
        fallbacks=[],
        per_message=False,
    )
    ...
    report_conv = ConversationHandler(
        ...
        states={
            ...
            REPORT_PHOTO: [
                MessageHandler(filters.PHOTO | filters.Document.IMAGE, report_photo),
    ...
    app.add_handler(MessageHandler(filters.PHOTO | filters.Document.IMAGE, photo_fallback))
...
async def _run_webhook_server(tg_app: Application, base_url: str, path: str, port: int):
    """
    Render webhook runner:
    - Starts PTB application (background update processing)
    - Exposes aiohttp server on PORT with:
        GET  /, /health, /healthz  -> ok
        POST /<path>               -> Telegram webhook
    """
    path = path.lstrip("/")
    webhook_url = f"{base_url.rstrip('/')}/{path}" if path else base_url.rstrip("/")
    ...
    async def telegram_webhook(request: web.Request) -> web.Response:
        ...
        try:
            ...
            log.warning("Failed to enqueue update: %s", e)
    ...
    if path:
        aio.router.add_post(f"/{path}", telegram_webhook)
    else:
        aio.router.add_post("/", telegram_webhook)

    runner = web.AppRunner(aio)
    await runner.setup()
    site = web.TCPSite(runner, host="0.0.0.0", port=port)
    await site.start()
    log.info("HTTP server listening on 0.0.0.0:%s (webhook path: /%s)", port, path)

    # wait forever, with proper shutdown on SIGTERM
...
def main():
    webhook_mode = (os.getenv("WEBHOOK_MODE") or "0").strip() == "1"

    if webhook_mode:
        base_url = (os.getenv("WEBHOOK_BASE_URL") or "").strip()
        path = (os.getenv("WEBHOOK_PATH") or "").strip()
        port = int((os.getenv("PORT") or "10000").strip() or "10000")

        if not base_url:
            raise RuntimeError("WEBHOOK_BASE_URL is empty (set it in Render Environment)")

        tg_app = build_app()
        log.info("Starting bot in webhook mode: %s /%s", base_url, path)
        asyncio.run(_run_webhook_server(tg_app, base_url=base_url, path=path, port=port))
    else:
        # Polling mode (local dev): keep old health server on PORT
        start_health_server_polling()
        tg_app = build_app()
